...
public class MeinNPStudentController {
    ...
    @FXML
    void cmdBackButton(ActionEvent event) throws IOException {
    ...
    @FXML
    void cmdProfilBearbeiten(ActionEvent event) throws IOException {
        //Öffnen des Bearbeitungsfensters
        stage = (Stage) pane.getScene().getWindow();
        stage.close();
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/NutzerprofilStudentBearbeiten.fxml"));
        Stage stage = new Stage();
        stage.setScene(new Scene(loader.load()));
        NPStudentBearbeitenController controller = loader.getController();
        controller.setIDLabel(currentUserID);
        controller.init();
        stage.show();
    }
    ...
    //Initialisierung mit Aenderung der JavaFX Texte
